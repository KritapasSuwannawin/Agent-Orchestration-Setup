...
> For global coding conventions and stack info, see `.cursor/rules/agent-instructions.mdc`.  
> This file is the reference manual for the agent orchestration system.

## Agents

Agents are available at `.cursor/agents/` as `{canonical-id}.md` (e.g. `project-manager.md`, `developer.md`). Frontmatter `name` matches the kebab-case id. Use those ids in docs, handoffs, Task spawns, and `/name` invocation. Human-readable titles in agent body text are labels only.
...
- Store task-scoped evidence under `.cursor/docs/{feature}/{task}/artifacts/`.
...
User
 └─▶ project-manager
   ├─▶ architect                            → .cursor/docs/{feature}/architecture.md
   ├─▶ [plan summary + task breakdown]      → .cursor/docs/{feature}/tasks.md
   └─▶ per task:
     ├─▶ designer-ui-ux (skip if task has no user-facing UI)    → .cursor/docs/{feature}/{task}/ui-spec.md
     ├─▶ developer
     │     ├─▶ [contract gate for full-stack tasks]             → .cursor/docs/{feature}/{task}/contract.md
     │     ├─▶ implementation + unit/integration/E2E tests
     │     └─▶ [code review + test review by developer]         → .cursor/docs/{feature}/{task}/dev-summary.md
     └─▶ tester
       └─▶ [functional + security + performance + usability review, QA verdict] → .cursor/docs/{feature}/{task}/qa-report.md
         └─▶ PASS → next task
...
All agent-produced documents are stored in `.cursor/docs/` following this structure:
...
- Paths are **always relative to the workspace root**. The `.cursor/` folder path must never be changed without updating every agent file and every skill file that references it.
...
**Docs:**
- Tasks: `.cursor/docs/{feature}/tasks.md` → Task {id}
- Architecture: `.cursor/docs/{feature}/architecture.md`
- UI Spec: `.cursor/docs/{feature}/{task}/ui-spec.md` or `N/A`
- Contract: `.cursor/docs/{feature}/{task}/contract.md` or `N/A`
- Dev Summary: `.cursor/docs/{feature}/{task}/dev-summary.md` or `N/A`
...
All agents reference skills from `.cursor/skills/` in explicit workflow steps rather than optional top-level lists. Keep this matrix aligned with the agent files.
