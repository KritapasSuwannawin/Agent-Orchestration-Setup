...
| --------------------------- | ---------------------------- | -------------------------------------------- |
| Architecture                | `architect`                  | `.github/docs/{feature}/architecture.md`     |
| Task breakdown              | _(you)_                      | `.github/docs/{feature}/tasks.md`            |
| UI spec (if user-facing UI) | `designer-ui-ux`             | `.github/docs/{feature}/{task}/ui-spec.md`   |
| Implementation              | `developer`                  | code + `contract.md` + `dev-summary.md`      |
| QA                          | `tester`                     | `.github/docs/{feature}/{task}/qa-report.md` |

- **MUST** wait for each Task result before the next step in the chain (unless parallelizing explicitly independent work).
- **MUST NOT** use **Write**, **StrReplace**, or **EditNotebook** on `petsy-backend/` or `petsy-frontend/` — only under `.github/docs/`.
...
5. **For each task**, execute the following loop:
   ...
   - **Task** → `tester` with: task description + implementation summary + instruction to apply `.github/skills/definition-of-done/SKILL.md`
