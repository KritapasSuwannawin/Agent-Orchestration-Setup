...
Agents are available at `.github/agents/` as `{canonical-id}.md` (e.g. `project-manager.md`, `developer.md`). Frontmatter `name` matches the kebab-case id. Use those ids in docs, handoffs, Task spawns, and `/name` invocation. Human-readable titles in agent body text are labels only.
