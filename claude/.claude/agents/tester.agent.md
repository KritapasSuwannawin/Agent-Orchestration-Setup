...
4. **Apply the security review lens** when the task touches new endpoints, auth, permissions, sensitive data, input validation, or browser-visible security behavior. Read `.claude/skills/security-review/SKILL.md` before doing so.
5. **Apply the performance review lens** when the task changes rendering, large client-side data, queries, payload size, caching, or backend throughput concerns. Read `.claude/skills/performance-review/SKILL.md` before doing so.
6. **Apply the usability and accessibility review lens** when the task changes user-facing UI. Read `.claude/skills/usability-review/SKILL.md`, `.claude/skills/ui-design/SKILL.md`, and `.claude/skills/accessibility/SKILL.md` before doing so.
...
10. **Apply the definition of done** — read `.claude/skills/definition-of-done/SKILL.md` before issuing the verdict.
...
Write a file named `qa-report.md` to `.claude/docs/{feature}/{task}/qa-report.md` with the following sections:
...
(from .claude/skills/definition-of-done/SKILL.md)
