...
> For global coding conventions and stack info, see `.claude/rules/agent-instructions.md`.  
...
- Store task-scoped evidence under `.claude/docs/{feature}/{task}/artifacts/`.
...
User
 └─▶ project-manager
   ├─▶ architect                            → .claude/docs/{feature}/architecture.md
   ├─▶ [plan summary + task breakdown]      → .claude/docs/{feature}/tasks.md
   └─▶ per task:
     ├─▶ designer-ui-ux (skip if task has no user-facing UI)    → .claude/docs/{feature}/{task}/ui-spec.md
     ├─▶ developer
     │     ├─▶ [contract gate for full-stack tasks]             → .claude/docs/{feature}/{task}/contract.md
     │     ├─▶ implementation + unit/integration/E2E tests
     │     └─▶ [code review + test review by developer]         → .claude/docs/{feature}/{task}/dev-summary.md
     └─▶ tester
       └─▶ [functional + security + performance + usability review, QA verdict] → .claude/docs/{feature}/{task}/qa-report.md
         └─▶ PASS → next task
...
All agent-produced documents are stored in `.claude/docs/` following this structure:
...
- Paths are **always relative to the workspace root**. The `.claude/` folder path must never be changed without updating every agent file and every skill file that references it.
...
All agents reference skills from `.claude/skills/` in explicit workflow steps rather than optional top-level lists. Keep this matrix aligned with the agent files.
